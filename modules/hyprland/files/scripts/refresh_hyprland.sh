...
LUA_OUTPUT="$HOME/.config/retro/themes/variables.lua"
CONF_OUTPUT="$HOME/.config/retro/themes/variables.conf"
TEMP_FILE=$(mktemp)
...
LOCK_FONT=$(get_var "LOCK_FONT" "JetBrainsMono Nerd Font")
LOCK_SCALE=$(get_var "LOCK_SCALE" "1.0")
LOCK_BACKGROUND_BLUR_SIZE=$(get_var "LOCK_BACKGROUND_BLUR_SIZE" "12")
LOCK_BLUR_PASSES=$(get_var "LOCK_BLUR_PASSES" "4")

SCALED_CLOCK_SIZE=$(awk "BEGIN { printf \"%.0f\", 70 * $LOCK_SCALE }")
SCALED_DATE_SIZE=$(awk "BEGIN { printf \"%.0f\", 14 * $LOCK_SCALE }")
SCALED_LAYOUT_SIZE=$(awk "BEGIN { printf \"%.0f\", 14 * $LOCK_SCALE }")
SCALED_FPRINT_SIZE=$(awk "BEGIN { printf \"%.0f\", 16 * $LOCK_SCALE }")
SCALED_AVATAR_SIZE=$(awk "BEGIN { printf \"%.0f\", 100 * $LOCK_SCALE }")
SCALED_INPUT_WIDTH=$(awk "BEGIN { printf \"%.0f\", 300 * $LOCK_SCALE }")
SCALED_INPUT_HEIGHT=$(awk "BEGIN { printf \"%.0f\", 60 * $LOCK_SCALE }")
SCALED_CLOCK_Y=$(awk "BEGIN { printf \"%.0f\", 60 * $LOCK_SCALE }")
SCALED_DATE_Y=$(awk "BEGIN { printf \"%.0f\", 120 * $LOCK_SCALE }")
SCALED_AVATAR_Y=$(awk "BEGIN { printf \"%.0f\", -40 * $LOCK_SCALE }")
SCALED_INPUT_Y=$(awk "BEGIN { printf \"%.0f\", 20 * $LOCK_SCALE }")
SCALED_FPRINT_Y=$(awk "BEGIN { printf \"%.0f\", -80 * $LOCK_SCALE }")
SCALED_LAYOUT_X=$(awk "BEGIN { printf \"%.0f\", 20 * $LOCK_SCALE }")
SCALED_LAYOUT_Y=$(awk "BEGIN { printf \"%.0f\", 20 * $LOCK_SCALE }")
...
if ! mv "$TEMP_FILE" "$LUA_OUTPUT"; then
    rm -f "$TEMP_FILE"
    exit 1
fi
chmod 644 "$LUA_OUTPUT"
...
\$SHADOW = $SHADOW_BOOL
\$BLUR = $BLUR_BOOL
\$ANIMATIONS = $ANIMATIONS_BOOL
\$LOCK_FONT = $LOCK_FONT
\$LOCK_CLOCK_SIZE = $SCALED_CLOCK_SIZE
\$LOCK_DATE_SIZE = $SCALED_DATE_SIZE
\$LOCK_LAYOUT_SIZE = $SCALED_LAYOUT_SIZE
\$LOCK_FPRINT_SIZE = $SCALED_FPRINT_SIZE
\$LOCK_AVATAR_SIZE = $SCALED_AVATAR_SIZE
\$LOCK_CLOCK_Y = $SCALED_CLOCK_Y
\$LOCK_DATE_Y = $SCALED_DATE_Y
\$LOCK_AVATAR_Y = $SCALED_AVATAR_Y
\$LOCK_INPUT_Y = $SCALED_INPUT_Y
\$LOCK_FPRINT_Y = $SCALED_FPRINT_Y
\$LOCK_LAYOUT_X = $SCALED_LAYOUT_X
\$LOCK_LAYOUT_Y = $SCALED_LAYOUT_Y
\$LOCK_BACKGROUND_BLUR_SIZE = $LOCK_BACKGROUND_BLUR_SIZE
\$LOCK_BLUR_PASSES = $LOCK_BLUR_PASSES
EOF
chmod 644 "$CONF_OUTPUT"
