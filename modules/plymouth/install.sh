...
set_plymouth_theme() {
    ...
    if command -v plymouth-set-default-theme >/dev/null 2>&1; then
        if plymouth-set-default-theme "$theme_name" >/dev/null 2>&1; then
            rx_log "success" "Plymouth theme set to $theme_name"
        else
            rx_log "warn" "Could not set Plymouth theme via command"

            local plymouth_conf="/etc/plymouth/plymouthd.conf"
            if [[ -f $plymouth_conf ]]; then
                sed -i "s|^Theme=.*|Theme=$theme_name|" "$plymouth_conf"
                rx_log "success" "Plymouth theme set via config file"
            fi
        fi
    fi
}
