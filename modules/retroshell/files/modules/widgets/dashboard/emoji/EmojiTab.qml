...
Rectangle {
    ...
    function loadEmojiData() {
        var dir = Qt.resolvedUrl("../../../../assets/clipboard").toString().replace("file://", "");
        emojiProcess.command = ["bash", "-c", "for f in '" + dir + "/emojis_'*.csv; do g=$(basename \"$f\" .csv | sed 's/^emojis_//'); sed \"s/^/$g|/\" \"$f\"; done"];
        emojiProcess.running = true;
    }

    function loadInitialEmojis() {
        emojisModel.clear();
        if (recentEmojis.length > 0 && searchText === "" && Config.tools.emojiShowRecent) {
            emojisModel.append({
    ...
    function loadRecentEmojis() {
        ...
        recentProcess.command = ["bash", "-c", "cat " + Quickshell.env("HOME") + "/.cache/retro/shell" + "/emojis.json 2>/dev/null || echo '[]'"];
    ...
    function saveRecentEmojis() {
        ...
        saveProcess.command = ["bash", "-c", "echo '" + jsonData.replace(/'/g, "'\\''") + "' > " + Quickshell.env("HOME") + "/.cache/retro/shell" + "/emojis.json"];
            ...
            onStreamFinished: {
                try {
                    ...
                    for (var i = 0; i < lines.length; i++) {
                        ...
                        var group = line.substring(0, sepIdx);
                        var rest = line.substring(sepIdx + 1);
                        var firstSpace = rest.indexOf(" ");
                        if (firstSpace < 0) continue;
                        var emoji = rest.substring(0, firstSpace);
                        var namePart = rest.substring(firstSpace + 1);
                        var smallIdx = namePart.indexOf("<small>");
                        var name, keywords = [];
                        if (smallIdx >= 0) {
                            name = namePart.substring(0, smallIdx).trim();
                            var m = namePart.substring(smallIdx).match(/\(([^)]*)\)/);
                            if (m) keywords = m[1].split(",").map(function(k) { return k.trim(); });
                        } else {
                            name = namePart.trim();
                        }
                        var slug = name.replace(/[^a-z0-9]+/gi, "_").toLowerCase();
                        var search = name + " " + keywords.join(" ");
                        data.push({
                            emoji: emoji,
                            name: name,
                            slug: slug,
                    ...
                    emojiData = data;
                    // Sort: smileys_emotion first, then people_body, then rest
                    var groupOrder = { smileys_emotion: 0, people_body: 1 };
                    emojiData.sort(function(a, b) {
                        var oa = groupOrder[a.group] !== undefined ? groupOrder[a.group] : 10;
                        var ob = groupOrder[b.group] !== undefined ? groupOrder[b.group] : 10;
                        return oa - ob;
                    });
                    loadInitialEmojis();
                } catch (e) {
    ...
    Item {
        ...
        Row {
            ...
            StyledRect {
                ...
                visible: recentEmojis.length > 0 && searchText === "" && Config.tools.emojiShowRecent
