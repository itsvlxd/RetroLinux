...
FocusScope {
    ...
    property var filteredWallpapers: {
        ...
        if (activeFilters.length > 0) {
            var subfolderFilters = activeFilters.filter(function(f) { return f.startsWith("subfolder_"); });
            var typeFilters = activeFilters.filter(function(f) { return !f.startsWith("subfolder_"); });
            ...
            wallpapers = wallpapers.filter(function (path) {
                ...
                // Must match subfolder filter (if any active)
                if (subfolderFilters.length > 0) {
                    var matchesSubfolder = false;
                    for (var i = 0; i < subfolderFilters.length; i++) {
                        if (subfolder === subfolderFilters[i].replace("subfolder_", "")) {
