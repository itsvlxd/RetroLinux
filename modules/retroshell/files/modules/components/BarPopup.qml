...
PopupWindow {
    ...
    // When true, the popup window grabs mouse+keyboard (Qt.Popup flag).
    // Required for text inputs inside the popup to receive keystrokes.
    ...
    // Qt.Popup flag (keyboard+mouse grab) when enabled, so text inputs work.
    grabFocus: root.grabKeyboard
