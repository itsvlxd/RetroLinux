...
PanelWindow {
    ...
    property bool timedMode: GlobalStates.screenshotTimedMode

    onTimedModeChanged: {
        if (GlobalStates.screenshotTimedMode !== timedMode) {
            GlobalStates.screenshotTimedMode = timedMode
        }
    }
    ...
    Timer {
        ...
        interval: 120
        repeat: false
        onTriggered: Screenshot.freezeScreen()
    }
    ...
    function open() {
        ...
        // Delay freeze so compositor renders without bar/notch first
        freezeDelay.start();
    }
    ...
    function executeCapture() {
        if (timedMode) {
            GlobalStates.pendingScreenshotMode = screenshotPopup.currentMode;
            GlobalStates.pendingScreenshotScreen = screenshotPopup.targetScreen.name;
            GlobalStates.countdownMode = "screenshot";
            GlobalStates.countdownOverlayVisible = true;
            close();
            return;
        }
    ...
    FocusScope {
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    TapHandler {
                        onTapped: {
                            if (timedMode) {
                                Screenshot.selectionX = modelData.at[0];
                                Screenshot.selectionY = modelData.at[1];
                                Screenshot.selectionW = modelData.size[0];
                                Screenshot.selectionH = modelData.size[1];
                                GlobalStates.pendingScreenshotMode = "region";
                                GlobalStates.countdownMode = "screenshot";
                                GlobalStates.countdownOverlayVisible = true;
                                close();
                            } else {
        ...
        MouseArea {
            ...
            onClicked: {
                if (screenshotPopup.currentMode === "screen") {
                    if (timedMode) {
                        GlobalStates.pendingScreenshotMode = "screen";
                        GlobalStates.pendingScreenshotScreen = screenshotPopup.targetScreen.name;
                        GlobalStates.countdownMode = "screenshot";
                        GlobalStates.countdownOverlayVisible = true;
                        close();
        ...
        Rectangle {
            ...
            height: controlsRow.implicitHeight + 32
            ...
            RowLayout {
                ...
                StyledRect {
                    ...
                    variant: screenshotPopup.timedMode ? "primary" : "common"
                    ...
                    MouseArea {
                        ...
                        onClicked: {
                            screenshotPopup.timedMode = !screenshotPopup.timedMode;
                            GlobalStates.screenshotTimedMode = screenshotPopup.timedMode;
                        }
                    }

                    HoverHandler {
                        onHoveredChanged: timerBtn.btnHovered = hovered
                    }

                    StyledToolTip {
                        visible: timerBtn.btnHovered
                        tooltipText: screenshotPopup.timedMode ? "Timed Capture: On" : "Timed Capture: Off"
                    }
                }
            }
        }
