...
PanelWindow {
    ...
    function getModes() {
        ...
        if (Config.tools.recordingPortalEnabled) {
            modes.push({
                name: "portal",
                icon: Icons.aperture,
                tooltip: "Portal (experimental)",
                enabled: true
            });
        }
        return modes;
    ...
    function open() {
        var defaultMode = "region";
        var defaultIndex = 3; // region (after audio + mic + separator)
        if (!ScreenRecorder.canRecordDirectly) {
            defaultMode = Config.tools.recordingPortalEnabled ? "portal" : "screen";
            defaultIndex = Config.tools.recordingPortalEnabled ? 6 : 5; // portal or screen
        }
        if (modeGrid)
            modeGrid.currentIndex = defaultIndex;
        screenrecordPopup.currentMode = defaultMode;
    ...
    function executeCapture() {
        if (screenrecordPopup.currentMode === "screen") {
            ...
            if (selectionRect.width > 0) {
                ...
                var mx = AxctlService.focusedMonitor ? AxctlService.focusedMonitor.x : 0;
                var my = AxctlService.focusedMonitor ? AxctlService.focusedMonitor.y : 0;
                x = x + mx;
                y = y + my;
        ...
        MouseArea {
            ...
            onReleased: {
                ...
                if (selectionRect.width > 5 && selectionRect.height > 5) {
                    ...
                    var mx = AxctlService.focusedMonitor ? AxctlService.focusedMonitor.x : 0;
                    var my = AxctlService.focusedMonitor ? AxctlService.focusedMonitor.y : 0;
                    var x = Math.round(selectionRect.x) + mx;
                    var y = Math.round(selectionRect.y) + my;
                    var regionStr = w + "x" + h + "+" + x + "+" + y;
