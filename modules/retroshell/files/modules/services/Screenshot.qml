...
QtObject {
    ...
    property string tempPathBase: "/tmp/retroshell_freeze"
    property string cropPath: "/tmp/retroshell_crop.png"
        ...
        onExited: exitCode => {
            ...
            if (exitCode === 0) {
                ...
                for (var i = 0; i < root.monitors.length; i++) {
                    ...
                    var path = root.tempPathBase + "_" + m.name + ".jpg";
                    root.monitorScreenshotReady(m.name, path);
                }
                root.screenshotCaptured(root.tempPathBase + "_ALL.jpg")
            } else {
    ...
    function processRegion(x, y, w, h) {
        ...
        var srcPath = root.tempPathBase + "_" + m.name + ".jpg";
    ...
    function processMonitorScreen(monitorName) {
        ...
        var srcPath = root.tempPathBase + "_" + monitorName + ".jpg";
        cropProcess.command = ["convert", srcPath, root.finalPath];
        cropProcess.running = true;
    }
