...
QtObject {
    ...
    function resolveOutputDir() {
        var cfgDir = (Config.tools.recordingsDir || "").trim();
        return cfgDir !== "" ? cfgDir : root.videosDir;
    }
    ...
    function toggleRecording() {
        if (isRecording) {
            ...
            var mode = Config.tools.recordingPortalEnabled ? "portal" : "screen";
            startRecording(false, false, mode, "");
        }
    }
    ...
    function startRecording(recordAudioOutput, recordAudioInput, mode, regionStr) {
        ...
        // Scale resolution (when not auto)
        var res = Config.tools.recordingResolution;
        if (res && res !== "auto" && res !== "") {
            cmd += " -s " + res;
        }
    ...
    property Process prepareProcess: Process {
        id: prepareProcess
        command: ["mkdir", "-p", root.resolveOutputDir()]
        onExited: exitCode => {
