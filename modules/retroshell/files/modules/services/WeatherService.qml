        ...
        stdout: StdioCollector {
            ...
            onStreamFinished: {
                ...
                if (raw.length > 0) {
                    try {
                        var jsonStr = raw;
                        // Check for CITY line from GeoIP auto-detection
                        var cityMatch = raw.match(/^CITY:(.+)\n/);
                        if (cityMatch) {
                            var detected = cityMatch[1].trim();
                            if (detected && !Config.weather.location)
                                Config.weather.location = detected;
                            jsonStr = raw.substring(cityMatch[0].length);
                        }
