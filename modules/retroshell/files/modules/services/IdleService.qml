...
Singleton {
    ...
    property string lockCmd: Config.system.idle.general.lock_cmd ?? "retro shell lock"
    property string beforeSleepCmd: Config.system.idle.general.before_sleep_cmd ?? "loginctl lock-session"
    property string afterSleepCmd: Config.system.idle.general.after_sleep_cmd ?? "retro shell screen on"

    readonly property bool hypridleActive: {
        try {
            return Quickshell.env("HYPRIDLE_ENABLE") === "true";
        } catch (e) {
            return false;
        }
    }
    ...
    function checkListeners() {
        // When hypridle is the active idle daemon, skip the internal listeners
        // to avoid duplicate timeouts firing.
        if (hypridleActive) return;
