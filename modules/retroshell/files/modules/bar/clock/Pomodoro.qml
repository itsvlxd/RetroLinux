...
Item {
    ...
    ColumnLayout {
        ...
        font.pixelSize: Styling.fontSize(14)
                    ...
                    TimerInput {
                        ...
                        onValueUpdated: val => {
                            let minPart = Math.floor(root.timeLeft / 60);
                            let carriedMin = Math.floor(val / 60);
                            let secPart = val % 60;
                            let newSeconds = (minPart + carriedMin) * 60 + secPart;
                            root.timeLeft = newSeconds;
                            if (!root.isRunning) {
                                root.totalTime = newSeconds;
                                if (root.isWorkSession) Config.system.pomodoro.workTime = newSeconds;
    ...
    component TimerInput: TextField {
        ...
        text: ""
        Component.onCompleted: text = value.toString().padStart(2, '0')
        ...
        Keys.onPressed: (event) => {
            if (event.key >= Qt.Key_0 && event.key <= Qt.Key_9) {
                let digit = String(event.key - Qt.Key_0);
                let current = text.length ? text : "00";
                text = (current + digit).slice(-2);
                tIn.valueUpdated(parseInt(text));
                event.accepted = true;
                return;
            }
            if (event.key === Qt.Key_Backspace) {
                let cur = text || "00";
                text = ("0" + cur).slice(0, 2);
                tIn.valueUpdated(parseInt(text));
                event.accepted = true;
                return;
            }
