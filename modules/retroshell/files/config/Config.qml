...
Singleton {
    ...
    FileView {
        id: toolsLoader
        path: root.configDir + "/tools.json"
        atomicWrites: true
        watchChanges: true
        onLoaded: {
            if (!root.toolsReady) {
                validateModule("tools", toolsLoader, ToolsDefaults.data, () => {
        ...
        onPathChanged: reload()
        onAdapterUpdated: {
            if (root.toolsReady && !root.pauseAutoSave) {
                toolsLoader.writeAdapter();
            }
        }

        adapter: JsonAdapter {
            property int screenshotCountdown: 5
            property bool previewCountdown: true
            property bool screenshotTimerEnabled: false
            property string screenshotsDir: ""
            property string recordingsDir: ""
            property string recordingResolution: "auto"
    ...
    FileView {
        ...
        adapter: JsonAdapter {
            property string clockFont: "League Gothic"
            property int clockFontSize: 240
            property string clockColor: "primaryFixed"
            property string clockMinutesColor: "outline"
            property int clockDateFontSize: 14
            property string clockDateColor: "primaryFixedDim"
            property string clockPosition: "center"
            property string clockStyle: "split"
            property string passwordPosition: "bottom"
            property string musicPosition: "bottom-left"
            property string weatherPosition: "bottom-left"
            property string powerPosition: "bottom-right"
        }
    }
        ...
        adapter: JsonAdapter {
            ...
            property JsonObject idle: JsonObject {
                property JsonObject general: JsonObject {
                    property string lock_cmd: "retro shell lock"
                    property string before_sleep_cmd: "loginctl lock-session"
                    property string after_sleep_cmd: "retro shell screen on"
                }
                property list<var> listeners: [
                    {
                        "timeout": 150,
                        "onTimeout": "retro shell brightness 10 -s",
                        "onResume": "retro shell brightness -r"
                    },
                    {
                        "timeout": 900,
                        "onTimeout": "retro shell screen off",
                        "onResume": "retro shell screen on"
                    },
                    {
                        "timeout": 1200,
                        "onTimeout": "retro shell suspend"
                    }
                ]
            }
            property JsonObject ocr: JsonObject {
                ...
                property bool deu: false
                property bool fra: false
                property bool pol: false
                property bool ron: false
                property bool swe: false
            }
    ...
    function repairKeybinds() {
        ...
        try {
            ...
            // Ensure retroshell structure exists
            if (!current.retroshell) {
                current.retroshell = {};
                needsUpdate = true;
            }

            // Migrate nested to flat structure
            if (current.retroshell.dashboard && typeof current.retroshell.dashboard === "object" && !current.retroshell.dashboard.modifiers) {
                console.log("Migrating nested retroshell binds to flat structure...");
                const nested = current.retroshell.dashboard;
                
                // Map old names to new names and update arguments
                if (nested.widgets) {
                    current.retroshell.launcher = nested.widgets;
                    current.retroshell.launcher.argument = "retro shell run launcher";
                    current.retroshell.launcher.action = createAction(current.retroshell.launcher);
                }
                if (nested.dashboard) {
                    current.retroshell.dashboard = nested.dashboard;
                    current.retroshell.dashboard.argument = "retro shell run dashboard";
                    current.retroshell.dashboard.action = createAction(current.retroshell.dashboard);
                }
                if (nested.assistant) {
                    current.retroshell.assistant = nested.assistant;
                    current.retroshell.assistant.argument = "retro shell run assistant";
                    current.retroshell.assistant.action = createAction(current.retroshell.assistant);
                }
                if (nested.clipboard) {
                    current.retroshell.clipboard = nested.clipboard;
                    current.retroshell.clipboard.argument = "retro shell run clipboard";
                    current.retroshell.clipboard.action = createAction(current.retroshell.clipboard);
                }
                if (nested.emoji) {
                    current.retroshell.emoji = nested.emoji;
                    current.retroshell.emoji.argument = "retro shell run emoji";
                    current.retroshell.emoji.action = createAction(current.retroshell.emoji);
                }
                if (nested.notes) {
                    current.retroshell.notes = nested.notes;
                    current.retroshell.notes.argument = "retro shell run notes";
                    current.retroshell.notes.action = createAction(current.retroshell.notes);
                }
                if (nested.tmux) {
                    current.retroshell.tmux = nested.tmux;
                    current.retroshell.tmux.argument = "retro shell run tmux";
                    current.retroshell.tmux.action = createAction(current.retroshell.tmux);
                }
                if (nested.wallpapers) {
                    current.retroshell.wallpapers = nested.wallpapers;
                    current.retroshell.wallpapers.argument = "retro shell run wallpapers";
                    current.retroshell.wallpapers.action = createAction(current.retroshell.wallpapers);
                }

                // Remove the old nested object
                delete current.retroshell.dashboard;
                needsUpdate = true;
            }

            if (!current.retroshell.system) {
                current.retroshell.system = {};
                needsUpdate = true;
            }

            // Get default binds from adapter
            const adapter = keybindsLoader.adapter;
            if (!adapter || !adapter.retroshell) return;
            ...
            // Check retroshell core binds
            const retroshellKeys = ["launcher", "dashboard", "assistant", "clipboard", "emoji", "notes", "tmux", "wallpapers"];
            for (const key of retroshellKeys) {
                if (!current.retroshell[key] && adapter.retroshell[key]) {
                    console.log("Adding missing retroshell bind:", key);
                    current.retroshell[key] = createCleanBind(adapter.retroshell[key]);
                    needsUpdate = true;
                } else if (current.retroshell[key] && !current.retroshell[key].action) {
                    current.retroshell[key].action = createAction(current.retroshell[key]);
                    delete current.retroshell[key].dispatcher;
                    delete current.retroshell[key].argument;
                    delete current.retroshell[key].flags;
                    needsUpdate = true;
                }
            }
            ...
            for (const key of systemKeys) {
                if (!current.retroshell.system[key] && adapter.retroshell.system && adapter.retroshell.system[key]) {
                    console.log("Adding missing system bind:", key);
                    current.retroshell.system[key] = createCleanBind(adapter.retroshell.system[key]);
                    needsUpdate = true;
                } else if (current.retroshell.system[key] && !current.retroshell.system[key].action) {
                    current.retroshell.system[key].action = createAction(current.retroshell.system[key]);
                    delete current.retroshell.system[key].dispatcher;
                    delete current.retroshell.system[key].argument;
                    delete current.retroshell.system[key].flags;
                    needsUpdate = true;
                }
            }
    ...
    FileView {
        ...
        adapter: JsonAdapter {
            property JsonObject retroshell: JsonObject {
                property JsonObject launcher: JsonObject {
                    property list<string> modifiers: ["SUPER"]
                    property string key: "Super_L"
                property var action: ({ "id": "retroshell.launcher", "args": {} })
            }
            property JsonObject dashboard: JsonObject {
                property list<string> modifiers: ["SUPER"]
                property string key: "D"
                property var action: ({ "id": "retroshell.dashboard", "args": {} })
            }
            property JsonObject assistant: JsonObject {
                property list<string> modifiers: ["SUPER"]
                property string key: "A"
                property var action: ({ "id": "retroshell.assistant", "args": {} })
            }
            property JsonObject clipboard: JsonObject {
                property list<string> modifiers: ["SUPER"]
                property string key: "V"
                property var action: ({ "id": "retroshell.clipboard", "args": {} })
            }
            property JsonObject emoji: JsonObject {
                property list<string> modifiers: ["SUPER"]
                property string key: "PERIOD"
                property var action: ({ "id": "retroshell.emoji", "args": {} })
            }
            property JsonObject notes: JsonObject {
                property list<string> modifiers: ["SUPER"]
                property string key: "N"
                property var action: ({ "id": "retroshell.notes", "args": {} })
            }
            property JsonObject tmux: JsonObject {
                property list<string> modifiers: ["SUPER"]
                property string key: "T"
                property var action: ({ "id": "retroshell.tmux", "args": {} })
            }
            property JsonObject wallpapers: JsonObject {
                property list<string> modifiers: ["SUPER"]
                property string key: "COMMA"
                property var action: ({ "id": "retroshell.wallpapers", "args": {} })
            }
            property JsonObject system: JsonObject {
                property JsonObject config: JsonObject {
                    ...
                    property var action: ({ "id": "retroshell.config", "args": {} })
                }
                ...
                property JsonObject overview: JsonObject {
                    ...
                    property var action: ({ "id": "retroshell.overview", "args": {} })
                }
                property JsonObject powermenu: JsonObject {
                    property list<string> modifiers: ["SUPER"]
                    property string key: "ESCAPE"
                    property var action: ({ "id": "retroshell.powermenu", "args": {} })
                }
                property JsonObject tools: JsonObject {
                    property list<string> modifiers: ["SUPER"]
                    property string key: "S"
                    property var action: ({ "id": "retroshell.tools", "args": {} })
                }
                property JsonObject screenshot: JsonObject {
                    property list<string> modifiers: ["SUPER", "SHIFT"]
                    property string key: "S"
                    property var action: ({ "id": "retroshell.screenshot", "args": {} })
                }
                property JsonObject screenrecord: JsonObject {
                    property list<string> modifiers: ["SUPER", "SHIFT"]
                    property string key: "R"
                    property var action: ({ "id": "retroshell.screenrecord", "args": {} })
                }
                property JsonObject lens: JsonObject {
                    property list<string> modifiers: ["SUPER", "SHIFT"]
                    property string key: "A"
                    property var action: ({ "id": "retroshell.lens", "args": {} })
                }
                property JsonObject reload: JsonObject {
                    property list<string> modifiers: ["SUPER", "ALT"]
                    property string key: "B"
                    property var action: ({ "id": "retroshell.reload", "args": {} })
                }
                property JsonObject quit: JsonObject {
                    property list<string> modifiers: ["SUPER", "CTRL", "ALT"]
                    property string key: "B"
                    property var action: ({ "id": "retroshell.quit", "args": {} })
                }
            }
            }
            // Default getters
            readonly property var defaultRetroshellBinds: {
                "retroshell": {
                    "launcher": { "modifiers": ["SUPER"], "key": "Super_L", "action": { "id": "retroshell.launcher", "args": {} } },
                    "dashboard": { "modifiers": ["SUPER"], "key": "D", "action": { "id": "retroshell.dashboard", "args": {} } },
                    "assistant": { "modifiers": ["SUPER"], "key": "A", "action": { "id": "retroshell.assistant", "args": {} } },
                    "clipboard": { "modifiers": ["SUPER"], "key": "V", "action": { "id": "retroshell.clipboard", "args": {} } },
                    "emoji": { "modifiers": ["SUPER"], "key": "PERIOD", "action": { "id": "retroshell.emoji", "args": {} } },
                    "notes": { "modifiers": ["SUPER"], "key": "N", "action": { "id": "retroshell.notes", "args": {} } },
                    "tmux": { "modifiers": ["SUPER"], "key": "T", "action": { "id": "retroshell.tmux", "args": {} } },
                    "wallpapers": { "modifiers": ["SUPER"], "key": "COMMA", "action": { "id": "retroshell.wallpapers", "args": {} } }
                },
                "system": {
                    "config": { "modifiers": ["SUPER", "SHIFT"], "key": "C", "action": { "id": "retroshell.config", "args": {} } },
                    "lockscreen": { "modifiers": ["SUPER"], "key": "L", "action": { "id": "system.lock", "args": {} } },
                    "overview": { "modifiers": ["SUPER"], "key": "TAB", "action": { "id": "retroshell.overview", "args": {} } },
                    "powermenu": { "modifiers": ["SUPER"], "key": "ESCAPE", "action": { "id": "retroshell.powermenu", "args": {} } },
                    "tools": { "modifiers": ["SUPER"], "key": "S", "action": { "id": "retroshell.tools", "args": {} } },
                    "screenshot": { "modifiers": ["SUPER", "SHIFT"], "key": "S", "action": { "id": "retroshell.screenshot", "args": {} } },
                    "screenrecord": { "modifiers": ["SUPER", "SHIFT"], "key": "R", "action": { "id": "retroshell.screenrecord", "args": {} } },
                    "lens": { "modifiers": ["SUPER", "SHIFT"], "key": "A", "action": { "id": "retroshell.lens", "args": {} } },
                    "reload": { "modifiers": ["SUPER", "ALT"], "key": "B", "action": { "id": "retroshell.reload", "args": {} } },
                    "quit": { "modifiers": ["SUPER", "CTRL", "ALT"], "key": "B", "action": { "id": "retroshell.quit", "args": {} } }
                }
            }

            function getAmbxstDefault(section, key) {
                if (defaultRetroshellBinds[section] && defaultRetroshellBinds[section][key]) {
                    const bind = defaultRetroshellBinds[section][key];
                    return {
            ...
            property list<var> custom: [
                ...
                },
                {
                    ...
                    "actions": [
                        {
                            "dispatcher": "exec",
                            "argument": "retroshell brightness +5",
                            "flags": "le",
                            "layouts": []
                        }
                    ],
                ...
                },
                {
                    ...
                    "actions": [
                        {
                            "dispatcher": "exec",
                            "argument": "hyprctl dispatch hl.dsp.dpms({ action: \"disable\" })",
                            "flags": "l",
                            "layouts": []
                        }
                    ],
                ...
                },
                {
                    ...
                    "actions": [
                        {
                            "dispatcher": "exec",
                            "argument": "hyprctl dispatch hl.dsp.dpms({ action: \"enable\" })",
                            "flags": "l",
                            "layouts": []
                        }
                    ],
    ...
    property QtObject tools: toolsLoader.adapter
