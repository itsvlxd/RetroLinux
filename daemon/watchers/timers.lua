return {
    ...
    start = function(engine)
        ...
        local pkg_min = tonumber(Watcher.get_var("RETRO_PKG_UPDATE_MIN", "60")) or 60
        local pkg_interval = pkg_min * 60

        local retro_min = tonumber(Watcher.get_var("RETRO_UPDATE_CHECK_MIN", "15")) or 15
        ...
        while true do
            ...
            if now - last_pkg_check > pkg_interval then
                ...
                if total > 0 and total >= thresh then
                    local sample_raw = Watcher.run_cmd("(checkupdates 2>/dev/null; " .. helper .. " -Qu 2>/dev/null) | head -n 3 | awk '{print $1}'")
                    local sample = ""
                    local count = 0
            ...
            if retro_dir and now - last_retro_check > retro_interval then
                ...
                if git_check == "yes" then
                    ...
                    local branch = Watcher.run_cmd("git -C '" .. retro_dir .. "' rev-parse --abbrev-ref HEAD 2>/dev/null")
                    local behind_str = Watcher.run_cmd("git -C '" .. retro_dir .. "' rev-list --count HEAD..origin/" .. branch .. " 2>/dev/null")
