...
setup_system() {
    if grep -q '^ID=retroarch' /etc/os-release 2>/dev/null; then
        return 0
    fi
    ...
    if [[ $current_hostname == "archlinux" ]]; then
        echo "retroarch" | sudo tee /etc/hostname >/dev/null
        sudo hostnamectl set-hostname retroarch
        current_hostname="retroarch"
    fi
