...
from settings.ui.bezier_canvas import AnimationPreview, BezierCanvas
...
class BezierEditorDialog:
    ...
    def _start_preview(self):
        """Switch the animation to settingsCurve for live preview."""
        ...
        self._anims.define_bezier("settingsCurve", self._editor.get_points())
        self._anims.preview(self._animation_name, eff_en, eff_sp, "settingsCurve", eff_st)
    ...
    def _on_curve_changed(self, x1, y1, x2, y2):
        ...
        if self._animation_name and self._anims is not None:
            self._start_preview()
            self._anims.define_bezier("settingsCurve", (x1, y1, x2, y2))
    ...
    def _on_editor_saved(self, curve_name):
        ...
        if self._animation_name and self._animation_page and self._anims is not None:
            ...
            # Re-apply settingsCurve for continued live preview
            eff = self._animation_page.anims.get_effective(self._animation_name)
            self._anims.define_bezier("settingsCurve", self._editor.get_points())
            self._anims.preview(self._animation_name, eff[0], eff[1], "settingsCurve", eff[3])
        if self._on_curve_saved:
