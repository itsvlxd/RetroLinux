...
from settings.core.undo import AnimationUndoEntry
from settings.data.bezier_data import get_curve_store
from settings.pages.section import SectionPage
from settings.ui.bezier_editor import BezierEditorDialog
from settings.ui.icons import ANIMATIONS_ICON
...
class AnimationsPage(SectionPage):
    ...
    def load_owned_names(self, saved_sections=None):
        """Load which animation names Retro Settings owns from the config file."""
    ...
    def get_animation_lines(self) -> tuple[list[str], set[str]]:
        """Return config lines for animations owned by Retro Settings or newly changed,
        ...
        for name, _, _, _ in ANIM_FLAT:
            ...
            if not state or not state.overridden:
                continue
            # Only include if Retro Settings owns it (was in our config) or the user changed it
