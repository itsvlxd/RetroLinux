...
class OverrideTracker:
    """Track which Retro Settings-managed binds override Hyprland's runtime binds.

    This allows Retro Settings to emit 'unbind' lines in the config for overridden
    binds, and to filter the Hyprland bind list to avoid showing duplicates.

    *managed_path* identifies settings's own config file so unbind lines
    ...
    (rather than calling back into settings modules) keeps this class
    portable — it has no other settings-side dependencies. ``None`` is
