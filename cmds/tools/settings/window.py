...
class RetroSettingsWindow(Adw.ApplicationWindow):
    ...
    def _update_sidebar_badges(self):
        ...
        if shell_bar_page is not None and shell_bar_page.is_dirty():
            counts["shell_bar"] = 1
        shell_desktop_page = getattr(self, "_shell_desktop_page", None)
        if shell_desktop_page is not None and shell_desktop_page.is_dirty():
            counts["shell_desktop"] = 1
        ...
        shell_notch_page = getattr(self, "_shell_notch_page", None)
        if shell_notch_page is not None and shell_notch_page.is_dirty():
            counts["shell_notch"] = 1
        shell_frame_page = getattr(self, "_shell_frame_page", None)
