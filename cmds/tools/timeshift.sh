...
cmd_timeshift() {
    ...
    case "$action" in
        ...
        *)
            ...
            rx_help_cmd "schedule [timeframe] [count|disable]" "Show or set snapshot retention schedule"
            rx_help_cmd "setup [-o key=val,...]" "Interactive or scripted setup wizard"
            rx_help_cmd "gui" "Open Timeshift graphical interface"
            rx_help_examples
            rx_help_example "retro timeshift status" "Show current config and snapshot count" "38"
            rx_help_example "retro timeshift list" "List all snapshots" "38"
            rx_help_example "retro timeshift create 'Before update'" "Create a snapshot with comment" "38"
            rx_help_example "retro timeshift schedule daily 5" "Keep 5 daily snapshots" "38"
            rx_help_example "retro timeshift schedule weekly disable" "Turn off weekly backups" "38"
            rx_help_example "retro timeshift setup" "Interactive setup wizard" "38"
            rx_help_example "retro timeshift setup -o device=/dev/sda1,daily=5,weekly=3,monthly=2,boot=true" "Non-interactive setup" "38"
            rx_help_example "retro timeshift gui" "Open GUI for manual management" "38"
            rx_help_spacer
            ;;
    esac
}
